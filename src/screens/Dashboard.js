...
const ENPTY_ITEM_SIZE = (SIZES.width - PLACES_ITEM_SIZE) / 2
...
return(
    ...
    onMomentumScrollEnd={(event) => {
        ...
        //Set Plase
        setPlaces([
            {id:-1}, ...dummyData.countries[position].places, {id:-2}
        ])
    }}
...
renderItem={({item, index}) => {
    ...
    if(index === 0 || index === countries.length - 1){
        return(
            <View style={{width:ENPTY_ITEM_SIZE}}/>
        )
    }else {
        return(
         <Animated.View 
...
<View style={{
    ...
    <Text style={{marginBottom:SIZES.radius*3.5,textAlign:'center', color:COLORS.white, ...FONTS.body3}}>{item.description}</Text>
